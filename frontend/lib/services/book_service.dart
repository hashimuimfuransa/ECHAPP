...
class BookService {
  ...
  // Fetch books by course ID
  Future<List<dynamic>> fetchBooksByCourse(String courseId, {int page = 1, int limit = 20}) async {
    try {
      final user = firebase_auth.FirebaseAuth.instance.currentUser;
      if (user == null) {
        throw Exception('User not authenticated');
      ...
      final uri = Uri.parse('${ApiConfig.baseUrl}/books/course/$courseId')
          .replace(queryParameters: queryParams);
