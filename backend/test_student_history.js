const https = require('https');

// Test the new student exam history endpoint
const testToken = 'eyJhbGciOiJSUzI1NiIsImtpZCI6ImY1MzMwMzNhMTMzYWQyM2UzOWY4YjQ4ZjMxYjQ3Y2I2YzYzYzA4YzUiLCJ0eXAiOiJKV1QifQ..TOKEN_SIGNATURE_HERE';

const options = {
  hostname: 'echappbackend.onrender.com',
  port: 443,
  path: '/api/exams/student/history',
  method: 'GET',
  headers: {
    'Authorization': `Bearer ${testToken}`,
    'Content-Type': 'application/json'
  }
};

console.log('Testing new student exam history endpoint...');
console.log('URL: https://echappbackend.onrender.com/api/exams/student/history');

const req = https.request(options, (res) => {
  console.log(`Status: ${res.statusCode}`);
  console.log(`Headers: ${JSON.stringify(res.headers)}`);
  
  let data = '';
  res.on('data', (chunk) => {
    data += chunk;
  });
  
  res.on('end', () => {
    console.log('Response body:');
    console.log(data);
    try {
      const jsonData = JSON.parse(data);
      console.log('Parsed JSON:');
      console.log(JSON.stringify(jsonData, null, 2));
    } catch (e) {
      console.log('Could not parse as JSON');
    }
  });
});

req.on('error', (error) => {
  console.error('Error:', error);
});

req.end();